class CulledMesh {
  ...
  // March over the volume
    ...
    const faces = []
    const x = [0, 0, 0]
    const B = [[false, true], [false, true], [false, true]] // Incrementally update bounds (this is a bit ugly)
    let n = -dims[0] * dims[1]
    for (B[2] = [false, true], x[2] = -1; x[2] < dims[2]; B[2] = [true, (++x[2] < dims[2] - 1)]) {
      for (n -= dims[0], B[1] = [false, true], x[1] = -1; x[1] < dims[1]; B[1] = [true, (++x[1] < dims[1] - 1)]) {
        for (n -= 1, B[0] = [false, true], x[0] = -1; x[0] < dims[0]; B[0] = [true, (++x[0] < dims[0] - 1)], ++n) {
          // Read current voxel and 3 neighboring voxels using bounds check results -> volume이 Material 이므로
